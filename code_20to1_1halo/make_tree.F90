...
!#endif
  ...
  !
  ! Parameters
  parameter (MAXNODES=1e+07) ! Absolute limit on number of nodes.
  !  
  ! Functions
  real sigma,deltc
  !
...
!write(0,*)'created storage arrays'
  ...
  nfraglev(:)=0
  jpfrag=-1
  !write(0,*) 'fin qui tutto bene 5, ', 'nfragmax = ', nfragmax
  ...
  alev(1)=1.0/(1.0+1.002) ! Enforce this!
  do ilev=1,nlev
...
!#ifdef INFO
           !write(0,*) 'make_tree(): INFO - increasing size inodemax: max_inode is ', max_inode
!#endif
           deallocate(wnode)
           deallocate(ml)
           deallocate(mr)
  ...
  do ilev=1,nlev-1
     do kfragp=jpfrag(ilev),jpfrag(ilev)+nfraglev(ilev)-1
        ...
        case (2:)
           ...
           !print*,"nchild1=",nchild1
           !print*,"size test=",size((/1.,2.,3./))
           !print*,"size merger=",size(MergerTree(jchild1:jchild1+nchild1-1)%mhalo)
           !print*,"merger=",MergerTree(jchild1:jchild1+nchild1-1)%mhalo
           !print*,"size indxch=",size(indxch)
